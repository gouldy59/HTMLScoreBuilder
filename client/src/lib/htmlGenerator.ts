...
function generatePagedComponentHTML(pagedComponent: PagedComponent, variables: Record<string, any>): string {
  ...
  switch (type) {
    ...
    case 'bar-chart':
      // Use Google Charts for bar chart in preview (same as builder)
      ...
      if (!barChartData && content.chartData) {
    ...
    case 'column-chart':
      // Use Google Charts for stacked column chart in preview (same as builder)
      ...
      if (content.chartData && Array.isArray(content.chartData) && content.chartData[0]?.segments) {
        ...
        columnChartColors = Array.from(colorSet);
      }
    ...
    case 'page-break':
      // Page breaks are invisible in preview - just create page boundary
